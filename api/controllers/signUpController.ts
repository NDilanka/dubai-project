  ...
  try {
    if (request.method == "POST") {
      ...
      if (data.firstName.length === 0) {
        return new Response(
          JSON.stringify({ message: "Please enter your first name!" }),
          {
            status: 400,
      ...
      if (data.lastName.length === 0) {
        return new Response(
          JSON.stringify({ message: "Please enter your last name!" }),
          {
            status: 400,
            headers: { "Content-Type": "application/json" },
          },
      ...
      if (data.email.length === 0) {
        return new Response(
          JSON.stringify({ message: "Please enter your email address!" }),
          {
            status: 400,
            headers: { "Content-Type": "application/json" },
          },
      ...
      if (data.currency.length === 0) {
        return new Response(
          JSON.stringify({ message: "Please enter the confirm password!" }),
          {
            status: 400,
            headers: { "Content-Type": "application/json" },
          },
      ...
      const result = await db.collection("tracking_data").updateOne(
        { _id: autoFXIds[0]._id },
        {
          $set: {
      ...
      if (!savedUser) {
        return new Response(
          JSON.stringify({ message: "Failed to create the user" }),
          {
            status: 500,
            headers: { "Content-Type": "application/json" },
          },
